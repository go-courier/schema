...
import (
	...
	"github.com/go-courier/schema/testdata/datalist"
...
func init() {
	...
	RestfulRouter.Register(courier.NewRouter(GetByUser{}))
...
type GetByUser struct {
	httpx.MethodGet `path:"/users"`
	datalist.ValueOrRangeOpt
}

func (req GetByUser) Output(ctx context.Context) (interface{}, error) {
	return &datalist.OrgUserDataList{}, nil
}
...
type GetByID struct {
	...
	Bytes         []byte       `name:"bytes,omitempty" in:"query"`
	Label         []string     `name:"label,omitempty" in:"query"`
	Name          string       `name:"name,omitempty" in:"query"`
	PtrString     *string      `name:"ptrString,omitempty" in:"query"`
}
...
func (DataProvider) ContextKey() interface{} {
	return "DataProvider"
}
...
func (r DataProvider) Output(ctx context.Context) (interface{}, error) {
	return &Data{
		ID: r.ID,
	}, nil
}
