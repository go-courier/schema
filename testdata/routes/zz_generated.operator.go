...
func (r *GetByID) FromRequestInfo(ri github_com_go_courier_httptransport_httpx.RequestInfo) error {
	...
	if values := ri.Values("query", "protocol"); len(values) > 0 {
		...
		r.Protocol = make([]github_com_go_courier_schema_testdata_a.Protocol, n)

		for i := range r.Protocol {
			if err := (&(github_com_go_courier_httptransport_transformers.TransformerPlainText{})).DecodeFrom(ri.Context(), github_com_go_courier_httptransport_transformers.NewStringReader(values[i]), &r.Protocol[i]); err != nil {
				errSet.AddErr(err, github_com_go_courier_schema_pkg_validator.Location("query"), "protocol", i)
			}
		}

	}
	if values := ri.Values("query", "bytes"); len(values) > 0 {

		if err := (&(github_com_go_courier_httptransport_transformers.TransformerPlainText{})).DecodeFrom(ri.Context(), github_com_go_courier_httptransport_transformers.NewStringReader(values[0]), &r.Bytes); err != nil {
			errSet.AddErr(err, github_com_go_courier_schema_pkg_validator.Location("query"), "bytes")
		}
									...
									Content: map[string]*github_com_go_courier_schema_pkg_openapi.MediaType{
										"application/json": &(github_com_go_courier_schema_pkg_openapi.MediaType{
											MediaTypeObject: github_com_go_courier_schema_pkg_openapi.MediaTypeObject{
												Schema: &(github_com_go_courier_schema_pkg_jsonschema.Schema{
													Reference: github_com_go_courier_schema_pkg_jsonschema.Reference{
														Refer: ref("github.com/go-courier/schema/testdata/datalist.OrgUserDataList"),
													},
												}),
											},
										}),
